...
public class ABCSongImporterStream implements TGSongStream {
	...
	public ABCSongImporterStream(TGSongStreamContext context) {
	...
	public void process() throws TGFileFormatException {
		try {			
			...
			TGSong song = this.parseSong(new ABCInputStream(stream, settings).readSong());
			
			this.context.setAttribute(TGDocumentContextAttributes.ATTRIBUTE_SONG, song);
		} catch (Exception e) {
			e.printStackTrace();
		}
