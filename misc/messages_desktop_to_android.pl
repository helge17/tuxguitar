...
if (!getopts('h') || $opt_h) {
  ...
  print "#  TuxGuitar/share/lang/messages_*.properties\n";
  print "# and writes them to the Android message files in\n";
  print "#  TuxGuitar-android/res/values-b+*/strings.xml\n";
  print "#\n";
...
my $msgs_en_android="TuxGuitar-android/res/values/strings.xml";
my $msgs_en_desktop="TuxGuitar/share/lang/messages.properties";
...
print "Write Android language files to TuxGuitar-android/res/:\n";

foreach my $msgs_XX_desktop (glob("TuxGuitar/share/lang/messages_*.properties")) {

  # Read and convert \uHHHH to utf8 using uconv
  @msgs_XX_desktop_content=`uconv -x hex-any $msgs_XX_desktop`;

  $msgs_XX_desktop =~ /TuxGuitar\/share\/lang\/messages_(.*).properties/;
  (my $lang=$1)=~s/_/+/g;
  print "$lang ";

  my $langdir="TuxGuitar-android/res/values-b+".$lang;
  mkdir $langdir;
