...
public class TGChangeTrackTuningAction extends TGActionBase {
	...
	public static final String ATTRIBUTE_KEEP_FRETS = "change-track-tuning.keep-frets";	// only meaningful if number of strings do not change
	...
	protected void processAction(TGActionContext context){
		...
		if( track != null ) {
			...
			if( strings != null ){
				boolean keepFrets = Boolean.TRUE.equals(context.getAttribute(ATTRIBUTE_KEEP_FRETS));
				if (keepFrets && (track.getStrings().size() == strings.size())) {
