	...
	public void previewChord(final TGChord chord) {
		...
		new Thread(new Runnable() {
			public void run() {
				...
				// FIXED: Adding "not played" notes to beat[], caused a delay beetween chord notes.
				/*
				int[][] beat = new int[stringCount][2];

				for (int i = 0; i < stringCount; i++) {
					int string = (stringCount - i);
					int value = chord.getFretValue(string - 1);
					if (value != -1) {
						beat[i][0] = getCurrentTrack().getOffset() + getCurrentTrack().getString(string).getValue() + value;
						beat[i][1] = TGVelocities.DEFAULT;
					}
				}
				*/
